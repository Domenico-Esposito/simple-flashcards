			...
			<Theme name={theme}>
				<PortalProvider>
					<ThemeProvider value={colorScheme === 'dark' ? DarkTheme : DefaultTheme}>
						{isLargeScreen ? <LargeScreenNavigation colorScheme={theme} topInset={insets.top} /> : <MobileNavigation />}
...
function Sidebar() {
	...
	const tabs = [
		...
		{ path: '/statistics', label: t('tab.statistics'), icon: 'bar-chart' as const },
				...
				return (
					<Pressable key={tab.path} onPress={() => router.push(tab.path as any)}>
						<XStack
							...
							<Text fontSize={15} fontWeight={isActive ? '600' : '400'} color={isActive ? '$color' : '$gray10'}>
								{tab.label}
							</Text>
						</XStack>
					</Pressable>
