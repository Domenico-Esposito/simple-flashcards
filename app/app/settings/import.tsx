...
export default function ImportScreen() {
	...
	const handleImportFromUrl = async () => {
		...
		try {
			...
			await loadDecks();
			Alert.alert('Importazione completata', `Mazzo "${deck.title}" importato con ${deck.flashcards.length} flashcard!`, [
	...
	const handleImportFromJson = async () => {
		if (!importJson.trim()) {
			Alert.alert('Errore', 'Inserisci il JSON del mazzo');
			return;
		}
		...
		try {
			...
			if (error instanceof SyntaxError) {
				Alert.alert('Errore', 'JSON non valido. Controlla la sintassi.');
			} else {
				Alert.alert('Errore', error instanceof Error ? error.message : "Errore durante l'importazione");
			}
		} finally {
      ...
      "answer": "Risposta 1"
								...
								<YStack gap="$2">
									...
									<Text fontSize={13} color="$secondary" marginLeft="$3">
										• <Text fontWeight="bold">question</Text>: Testo della domanda (HTML)
									</Text>
									<Text fontSize={13} color="$secondary" marginLeft="$3">
										• <Text fontWeight="bold">answer</Text>: Testo della risposta (HTML)
									</Text>
								</YStack>
							</View>
						)}
