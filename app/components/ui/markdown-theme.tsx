...
import type { SegmentComponentProps, SegmentComponentMap } from '@domenico-esposito/react-native-markdown-editor';
...
export function createSegmentComponents(scheme: ColorScheme): SegmentComponentMap {
	...
	const editorHeadingSizes: Record<string, number> = { '1': 32, '2': 26, '3': 22, '4': 20, '5': 18, '6': 17 };
	...
	function mergeStyle(type: string, meta?: Record<string, string>, overrides?: Record<string, any>): Record<string, any> {
		...
		const heading = meta?.lineContext === 'heading'
			? { fontSize: editorHeadingSizes[meta?.headingLevel ?? '1'] ?? 16, lineHeight: Math.ceil((editorHeadingSizes[meta?.headingLevel ?? '1'] ?? 16) * 1.5) }
			: undefined;
	...
	function createSegment(type: string, meta: Record<string, string> | undefined, children: React.ReactNode, style: Record<string, any>) {
		if (Platform.OS === 'web') {
	...
	const CodeSegment = ({ type, meta, children }: SegmentComponentProps) =>
		createSegment(type, meta, children, mergeStyle(type, meta, { color: c.code, backgroundColor: c.codeBg, fontSize: 16 }));
...
export function createRendererComponents(scheme: ColorScheme): RendererComponentMap {
	...
	const Root = ({ children, style }: RootRendererProps) => <View style={[rendererStyles.root, style]}>{children}</View>;

	const Paragraph = ({ children }: ParagraphRendererProps) => <Text style={[rendererStyles.paragraph, { color: c.text }]}>{children}</Text>;

	const TextComponent = ({ children }: TextRendererProps) => <Text style={{ color: c.text }}>{children}</Text>;

	function createHeading(fontSize: number) {
		const HeadingComponent = ({ children }: HeadingRendererProps<1 | 2 | 3 | 4 | 5 | 6>) => <Text style={{ fontSize, fontWeight: '700', color: c.text }}>{children}</Text>;
		HeadingComponent.displayName = `Heading`;
		return HeadingComponent;
	}

	const Bold = ({ children }: BoldRendererProps) => <Text style={[rendererStyles.bold, { color: c.text }]}>{children}</Text>;

	const Italic = ({ children }: ItalicRendererProps) => <Text style={[rendererStyles.italic, { color: c.text }]}>{children}</Text>;

	const Strikethrough = ({ children }: StrikethroughRendererProps) => <Text style={[rendererStyles.strikethrough, { color: c.text }]}>{children}</Text>;

	const InlineCode = ({ children }: InlineCodeRendererProps) => <Text style={[rendererStyles.inlineCode, { color: c.code, backgroundColor: c.codeBg }]}>{children}</Text>;

	const CodeBlock = ({ text, language }: CodeBlockRendererProps) => (
		<View style={[rendererStyles.codeBlock, { backgroundColor: c.codeBlockBg }]}>
			<Text style={[rendererStyles.codeBlockLanguage, { backgroundColor: c.codeBlockLabelBg, color: c.text }]}>{language?.trim() || 'Code'}</Text>
			<Text style={[rendererStyles.codeBlockText, { color: c.code }]}>{text}</Text>
		</View>
	);

	const Blockquote = ({ children }: BlockquoteRendererProps) => <View style={[rendererStyles.blockquote, { borderLeftColor: c.quoteBorder }]}>{children}</View>;

	const HorizontalRule = () => <View style={[rendererStyles.horizontalRule, { borderBottomColor: c.horizontalRule }]} />;

	const UnorderedList = ({ children }: UnorderedListRendererProps) => <View style={rendererStyles.list}>{children}</View>;

	const OrderedList = ({ children }: OrderedListRendererProps) => <View style={rendererStyles.list}>{children}</View>;
	...
	const Link = ({ children, href }: LinkRendererProps) => (
		<Text style={[rendererStyles.link, { color: c.link }]} onPress={() => href && Linking.openURL(href)}>
			{children}
		</Text>
	);
	...
	const ImageComponent = ({ src, alt, title }: ImageRendererProps) => (
		<View style={rendererStyles.imageContainer}>
			<Image source={{ uri: src }} style={rendererStyles.image} resizeMode="contain" accessibilityLabel={alt ?? title} />
