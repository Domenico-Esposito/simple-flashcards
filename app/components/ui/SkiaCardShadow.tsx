 ...
 */
export function SkiaCardShadow({ borderRadius, backgroundColor, shadows, children, style }: SkiaCardShadowProps) {
	...
	const spread = shadows.reduce((max, s) => Math.max(max, s.blur + Math.max(Math.abs(s.dx), Math.abs(s.dy))), 0);
		...
		<View style={[style, { overflow: 'visible' }]} onLayout={handleLayout}>
			{layout.width > 0 && layout.height > 0 && (
				<Canvas
					...
					pointerEvents="none">
					<RoundedRect x={spread} y={spread} width={layout.width} height={layout.height} r={borderRadius} color={backgroundColor}>
						{shadows.map((s, i) => (
